...
std::string FermiFockState::toString() const {
  std::string ans = "|";
  for (vector<bool>::const_iterator it = Nums.begin(); it != Nums.end(); ++it) {
    ans += " ";
...
std::string FermiState::toString() const {
  std::string ans = "";
  for (vector<pair<complex<double>, FermiFockState> >::const_iterator it = Terms.begin();
       it != Terms.end();
       ++it) {
    ans += "  (";
...
FermiState & FermiState::operator+=(FermiState const & rhs) {
  for (vector<pair<complex<double>, FermiFockState> >::const_iterator termIt =
           rhs.getBegin();
       termIt != rhs.getEnd();
       ++termIt) {
    *this += *termIt;
...
bool isZeroFS(pair<complex<double>, FermiFockState> term) {
  return std::abs(term.first) < std::pow(10, -12);
}

void FermiState::eraseZeros() {
  Terms.erase(std::remove_if(Terms.begin(), Terms.end(), isZeroFS), Terms.end());
}
...
complex<double> innerProduct(FermiState lhs, FermiState rhs) {
  complex<double> ans(0, 0);
  for (vector<pair<complex<double>, FermiFockState> >::const_iterator it = lhs.getBegin();
       it != lhs.getEnd();
       ++it) {
